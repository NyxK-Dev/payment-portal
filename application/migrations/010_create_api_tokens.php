...
class Migration_Create_api_tokens extends CI_Migration
{
    ...
    public function up()
    {
        ...
        $this->add_column_if_missing('api_tokens', 'created_at', [
            'type' => 'DATETIME',
            'null' => TRUE,
        ]);

        $this->add_unique_if_missing('api_tokens', 'uq_api_tokens_token_hash', array('token_hash'));
        $this->add_foreign_key_if_missing('api_tokens', 'fk_api_tokens_user', 'user_id', 'users', 'id', 'CASCADE', 'CASCADE');
    }
