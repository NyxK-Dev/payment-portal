...
class Migration_Create_invoices extends CI_Migration
{
    ...
    public function up()
    {
        ...
        $this->add_column_if_missing('invoices', 'issued_by', [
            'type'       => 'BIGINT',
            'constraint' => 20,
            'unsigned'   => TRUE,
            'null'       => TRUE,
        ]);
        $this->add_column_if_missing('invoices', 'updated_at', [
            'type' => 'DATETIME',
            'null' => TRUE,
        ]);

        $this->add_index_if_missing('invoices', 'idx_invoices_status_lookup_id', array('status_lookup_id'));
        $this->add_index_if_missing('invoices', 'idx_invoices_issued_by', array('issued_by'));
        $this->add_unique_if_missing('invoices', 'uq_invoices_invoice_no', array('invoice_no'));
        $this->add_foreign_key_if_missing('invoices', 'fk_invoices_order', 'order_id', 'orders', 'id', 'CASCADE', 'RESTRICT');
        $this->add_foreign_key_if_missing('invoices', 'fk_invoices_status_lookup', 'status_lookup_id', 'lookups', 'id', 'CASCADE', 'SET NULL');
        $this->add_foreign_key_if_missing('invoices', 'fk_invoices_issued_by', 'issued_by', 'users', 'id', 'CASCADE', 'SET NULL');
    }
