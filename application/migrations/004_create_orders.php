...
class Migration_Create_orders extends CI_Migration
{
    ...
    public function up()
    {
        ...
        $this->add_column_if_missing('orders', 'version', [
            'type'       => 'INT',
            'constraint' => 11,
            'default'    => 1,
            'null'       => TRUE,
        ]);
        $this->add_column_if_missing('orders', 'created_at', [
            'type' => 'DATETIME',
            'null' => TRUE,
        ]);
        $this->add_column_if_missing('orders', 'updated_at', [
            'type' => 'DATETIME',
            'null' => TRUE,
        ]);

        $this->add_index_if_missing('orders', 'idx_orders_status_lookup_id', array('status_lookup_id'));
        $this->add_unique_if_missing('orders', 'uq_orders_order_no', array('order_no'));
        $this->add_foreign_key_if_missing('orders', 'fk_orders_user', 'user_id', 'users', 'id', 'CASCADE', 'RESTRICT');
        $this->add_foreign_key_if_missing('orders', 'fk_orders_status_lookup', 'status_lookup_id', 'lookups', 'id', 'CASCADE', 'SET NULL');
    }
