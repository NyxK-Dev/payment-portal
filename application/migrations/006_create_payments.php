...
class Migration_Create_payments extends CI_Migration
{
    ...
    public function up()
    {
        if (!$this->db->table_exists('payments')) {
            ...
            $this->dbforge->add_key('payment_no');
            $this->dbforge->add_key('status_lookup_id');
            $this->dbforge->create_table('payments', TRUE);
        }

        $this->add_column_if_missing('payments', 'payment_no', [
            'type'       => 'VARCHAR',
            'constraint' => 100,
            'null'       => FALSE,
        ]);
        $this->add_column_if_missing('payments', 'version', [
            'type'       => 'INT',
            'constraint' => 11,
            'default'    => 1,
            'null'       => FALSE,
        ]);
        $this->add_column_if_missing('payments', 'failure_reason', [
            'type' => 'TEXT',
            'null' => TRUE,
        ]);
        $this->add_column_if_missing('payments', 'paid_at', [
            'type' => 'DATETIME',
            'null' => TRUE,
        ]);
        $this->add_column_if_missing('payments', 'created_at', [
            'type' => 'DATETIME',
            'null' => TRUE,
        ]);
        $this->add_column_if_missing('payments', 'updated_at', [
            'type' => 'DATETIME',
            'null' => TRUE,
        ]);

        $this->add_index_if_missing('payments', 'idx_payments_payment_no', array('payment_no'));
        $this->add_index_if_missing('payments', 'idx_payments_status_lookup_id', array('status_lookup_id'));
        $this->add_unique_if_missing('payments', 'uq_payments_payment_no', array('payment_no'));
        $this->add_foreign_key_if_missing('payments', 'fk_payments_order', 'order_id', 'orders', 'id', 'CASCADE', 'RESTRICT');
        $this->add_foreign_key_if_missing('payments', 'fk_payments_status_lookup', 'status_lookup_id', 'lookups', 'id', 'CASCADE', 'SET NULL');
    }
