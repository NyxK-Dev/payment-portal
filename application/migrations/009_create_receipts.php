...
class Migration_Create_receipts extends CI_Migration
{
    ...
    public function up()
    {
        ...
        $this->add_column_if_missing('receipts', 'issued_by', [
            'type'       => 'BIGINT',
            'constraint' => 20,
            'unsigned'   => TRUE,
            'null'       => TRUE,
        ]);
        $this->add_column_if_missing('receipts', 'updated_at', [
            'type' => 'DATETIME',
            'null' => TRUE,
        ]);

        $this->add_index_if_missing('receipts', 'idx_receipts_status_lookup_id', array('status_lookup_id'));
        $this->add_index_if_missing('receipts', 'idx_receipts_issued_by', array('issued_by'));
        $this->add_unique_if_missing('receipts', 'uq_receipts_receipt_no', array('receipt_no'));
        $this->add_foreign_key_if_missing('receipts', 'fk_receipts_invoice', 'invoice_id', 'invoices', 'id', 'CASCADE', 'RESTRICT');
        $this->add_foreign_key_if_missing('receipts', 'fk_receipts_status_lookup', 'status_lookup_id', 'lookups', 'id', 'CASCADE', 'SET NULL');
        $this->add_foreign_key_if_missing('receipts', 'fk_receipts_issued_by', 'issued_by', 'users', 'id', 'CASCADE', 'SET NULL');
    }
