...
ON DUPLICATE KEY UPDATE
    ...
    `status_lookup_id` = VALUES(`status_lookup_id`),
    `updated_at`     = VALUES(`updated_at`);
...
ON DUPLICATE KEY UPDATE
    ...
    `price`             = VALUES(`price`),
    `status_lookup_id`  = VALUES(`status_lookup_id`),
    `category_lookup_id`= VALUES(`category_lookup_id`),
    `updated_at`        = VALUES(`updated_at`);
...
-- Refunds
-- -----------------------------------------------------
INSERT INTO `refunds` (`payment_id`, `refund_no`, `stripe_refund_id`, `amount`, `currency`, `status_lookup_id`, `reason`, `created_by`, `approved_by`, `approved_at`, `created_at`, `updated_at`)
VALUES
    ((SELECT id FROM `payments` WHERE `payment_no` = 'PAY-20260709-0001'), 'REF-20260709-0001', 're_test_001', 19.99, 'USD', (SELECT id FROM `lookups` WHERE `group_id` = (SELECT id FROM `lookup_groups` WHERE `code` = 'refund_status') AND `code` = 'succeeded'), 'Partial refund for returned item', (SELECT id FROM `users` WHERE `email` = 'admin@example.com'), (SELECT id FROM `users` WHERE `email` = 'admin@example.com'), NOW(), NOW(), NOW())
ON DUPLICATE KEY UPDATE
...
INSERT INTO `receipts` (`invoice_id`, `order_id`, `receipt_no`, `amount`, `currency`, `status_lookup_id`, `issued_at`, `issued_by`, `created_at`, `updated_at`)
VALUES
    ((SELECT id FROM `invoices` WHERE `invoice_no` = 'INV-20260709-0001'), (SELECT id FROM `orders` WHERE `order_no` = 'ORD-20260709-0001'), 'RCT-20260709-0001', 179.98, 'USD', NULL, NOW(), (SELECT id FROM `users` WHERE `email` = 'admin@example.com'), NOW(), NOW())
ON DUPLICATE KEY UPDATE
    `amount`         = VALUES(`amount`),
    `currency`       = VALUES(`currency`),
    `issued_at`      = VALUES(`issued_at`),
...
VALUES
    ((SELECT id FROM `users` WHERE `email` = 'customer@example.com'), '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef', 'Customer API token', DATE_ADD(NOW(), INTERVAL 30 DAY), NOW())
ON DUPLICATE KEY UPDATE
    `name`       = VALUES(`name`),
    `expires_at` = VALUES(`expires_at`),
    `updated_at` = VALUES(`updated_at`);
...
INSERT INTO `email_logs` (`user_id`, `email_to`, `subject`, `status_lookup_id`, `response`, `sent_at`, `created_at`)
VALUES
    ((SELECT id FROM `users` WHERE `email` = 'admin@example.com'), 'customer@example.com', 'Welcome to Payment Portal', NULL, 'Message queued', NOW(), NOW())
ON DUPLICATE KEY UPDATE
